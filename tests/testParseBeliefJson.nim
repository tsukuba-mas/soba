...
#  rngInitializer((0..<agents).toSeq.mapIt(it + seed))
  ...
  test "when the key is -1 only":
    let belstr = "11110000"
    let json = """{"-1":"""" & belstr & """"}"""
    check json.parseBeliefJson(agents, atoms) == (0..<agents).mapIt((Id(it), toFormula(belstr))).toTable
  ...
  test "when all of the values are set":
    ...
    check json.parseBeliefJson(agents, atoms) == (0..<agents).mapIt((Id(it), toFormula(bels[it]))).toTable
