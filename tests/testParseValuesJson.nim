...
suite "Values Json Parser":
  ...
  test "when the key is -1 only":
    let values = (0..7).toSeq.mapIt(toRational(it, 7))
    ...
    check ids.allIt(actual[it] == expected[it].map(reduce))
  
  test "when all of the values are set":
    let values = @[
      (1..8).toSeq.mapIt(toRational(it, 100).reduce),
      (1..8).toSeq.mapIt(toRational(10 + it, 100).reduce),
      (1..8).toSeq.mapIt(toRational(20 + it, 100).reduce),
      (1..8).toSeq.mapIt(toRational(30 + it, 100).reduce),
      (1..8).toSeq.mapIt(toRational(40 + it, 100).reduce),
    ]
  ...
  test "-1 and agent id appear at the same time":
    let fixed = (0..7).toSeq.mapIt(toRational(it, 7))
    let values = @[
      fixed,
      (1..8).toSeq.mapIt(toRational(10 + it, 100)),
      (1..8).toSeq.mapIt(toRational(20 + it, 100)),
      fixed,
      (1..8).toSeq.mapIt(toRational(40 + it, 100)),
    ]
